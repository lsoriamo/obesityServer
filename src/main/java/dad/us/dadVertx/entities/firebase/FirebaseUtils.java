...
public class FirebaseUtils {
			...
			public void run() {
				try {
					...
					com.google.android.gcm.server.Message message = new com.google.android.gcm.server.Message.Builder()
							.collapseKey("message").timeToLive(3).delayWhileIdle(true).addData("messageBody", messageBody)
							.addData("messageTitle", messageTitle)
				...
				users.stream().forEach(user -> {
					ChatServer.getUserFirebaseToken(user.getIduser()).setHandler(handler -> {
						if (handler.succeeded() && handler.result() != null)
							FirebaseUtils.sendMessage(handler.result().getFirebase_token(), messageTitle, messageBody);
					});
				});
			}
		});
	...
	public static void sendMessageToGroup(Integer groupId, Long userException, ChatMessage message) {
		ChatServer.getGroupUsers(groupId).setHandler(new Handler<AsyncResult<List<User>>>() {
